...
#sys.path.append(os.path.abspath('/Data/Juan/contrastive_learning/tab-transformer-pytorch'))
...
#root_folder = '/Data/Juan/local_embeddings/TCGA-LUAD/clinical'
root_folder = '../local_embeddings/TCGA-LUAD/clinical'
...
df_blca = df[df['type'].str.contains("LUAD", na=False)]

# Load the Excel file with endpoints
#file_path = "/Data/Juan/HIV_TCGA.xls"
file_path = "HIV_TCGA.xls"
df_hiv_in= pd.read_excel(file_path, header=1)
df_hiv = df_hiv_in[df_hiv_in['Study'].str.contains("LUAD", na=False)]
...
def train_evaluate_wsi(params, train_loader, val_loader, save_dir):
    ...
    def create_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "p-solo-stratification-luad.png")):
...
def train_evaluate_end_to_end_fusion_late(
    ...
    time_points = np.linspace(times.min(), times.max(), 100)  # Time grid
    baseline_survival_at_times = np.array([kmf.predict(t) for t in time_points])
    ...
    # Step 5: Remove the last quartile of time points
    drop_last_times = 25  # Drop the last 25 time points (last quartile of 100)
    time_grid = time_points[:-drop_last_times]

    # Step 6: Compute IBS using EvalSurv
    # Transpose survival_df to match EvalSurv's expected format
    ev = EvalSurv(survival_df.T, times, events, censor_surv='km')
    ibs = ev.integrated_brier_score(time_grid)  # Compute IBS
    best_ibs = ibs
...
def train_evaluate_fold_clinical(params, train_loader, val_loader, categories, num_continuous, save_dir, device='cuda'):
    ...
    def create_clinical_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "c-solo-stratification-luad.png")):
...
def train_evaluate_end_to_end_fusion_vae(
    ...
    def create_vae_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "vae-stratification-luad.png")):
        """
...
def run_joint_experiment_standard_cv(clinical_data_yes_id, clinical_param_grid, embedding_dir, json_path, wsi_param_grid, model_type, cp_model, sr, filter_substring="DX1"):
    ...
    numeric_cols = [
        ...
        'patient.dlco_predictive_percent',
        'patient.post_bronchodilator_fev1_fvc_percent',
        'patient.post_bronchodilator_fev1_percent',
        'patient.pre_bronchodilator_fev1_fvc_percent',
        'patient.pre_bronchodilator_fev1_percent',
    ...
    categorical_cols = [
        ...
        'patient.ethnicity',
        'patient.anatomic_neoplasm_subdivision_other',
        'patient.eml4_alk_translocation_method',
        'patient.kras_mutation_result',
        'patient.kras_mutation_found',
        'patient.pulmonary_function_test_performed',
        'patient.egfr_mutation_result',
        'patient.location_in_lung_parenchyma',
        'patient.performance_status_scale_timing',
        'patient.eml4_alk_translocation_performed',
        'patient.egfr_mutation_performed',
    ...
    #orig_embeddings_dir = os.path.expanduser('/Data/Juan/local_embeddings/TCGA-LUAD-embeddings/')
    orig_embeddings_dir = os.path.abspath('../local_embeddings/TCGA-LUAD-embeddings/') 
...
if __name__ == "__main__":
    ...
    #embedding_dir = "/Data/Juan/local_embeddings/TCGA-LUAD-embeddings/"
    #json_path = "svs_patient_map_PFI_luad.json"
    embedding_dir = '../local_embeddings/TCGA-LUAD-embeddings/'    
    json_path = "svs_patient_map_PFI_luad.json"
