...
class CrossModalAttentionFusion(nn.Module):
    """
    Fuses clinical and WSI embeddings using bidirectional cross-attention.
    Inputs: h_clin, h_wsi: (B, 192)
    Output: h_fused: (B, 192)
    """
    def __init__(self, embed_dim=192, num_heads=2):
        super(CrossModalAttentionFusion, self).__init__()
        self.clin_to_wsi = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads, batch_first=True)
        self.wsi_to_clin = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, h_clin, h_wsi):
        # Add sequence dim: (B, 1, 192)
        h_clin = h_clin.unsqueeze(1)
        h_wsi = h_wsi.unsqueeze(1)

        # Clinical attends to WSI
        clin_attn, _ = self.clin_to_wsi(query=h_clin, key=h_wsi, value=h_wsi)  # (B, 1, 192)

        # WSI attends to Clinical
        wsi_attn, _ = self.wsi_to_clin(query=h_wsi, key=h_clin, value=h_clin)  # (B, 1, 192)

        # Concatenate and fuse
        h_combined = torch.cat([clin_attn, wsi_attn], dim=-1)  # (B, 1, 384)
        h_fused = self.fuse(h_combined).squeeze(1)             # (B, 192)
...
def train_evaluate_end_to_end_fusion_vae(
    ...
    # Build censoring distribution with Kaplan-Meier (for IPCW)
    kmf = KaplanMeierFitter()
    kmf.fit(val_times, event_observed=(~val_events))  # event=False => censored => observed for KM

    time_points = np.sort(np.unique(val_times))
    n = len(val_times)
    lower_bound = 5 / (np.sqrt(n) * np.log(n))  # A sample-size-based lower bound, if you want
    ...
    # Calculate Brier scores for each time point, then average => IBS
    brier_scores = []
    for t in time_points:
        # If your risk is a linear predictor, you might do S_t = exp(-H_t * risk).
        # But in your snippet, you used S_t = exp(-H_t) to match the first function's code.
        H_t = baseline_cumulative_hazard(t)
        S_t = np.exp(-H_t)  # per your snippet: “DO NOT multiply again by risks_val”

        # (val_times > t) is True if patient survived past t
        censor_mask = (val_events == 0)
        brier_t = np.mean(
            (S_t - (val_times > t)) ** 2 * np.where(censor_mask, ipcw_weights[t], 1)
        )
    ...
    # 4) Final C-index for the best model (you already have best_cindex, but you can recalc):
    final_cindex = concordance_index_censored(
        event_indicator=val_events,
        event_time=val_times,
        estimate=val_risks
    ...
    print(f"[Unified VAE Fusion] Best model reloaded. Final Val C-index = {final_cindex:.4f}, IBS = {ibs:.4f}")
    if os.path.exists(best_model_path):
        os.remove(best_model_path)

    return final_cindex, ibs
