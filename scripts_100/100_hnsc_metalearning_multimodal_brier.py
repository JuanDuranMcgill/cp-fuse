...
#root_folder = '/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-HNSC/tcga_hnsc_xml_clinical'
root_folder = '../local_embeddings/TCGA-HNSC/tcga_hnsc_xml_clinical'
...
def train_evaluate_wsi(params, train_loader, val_loader, save_dir):
    ...
    def create_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "p-solo-stratification-hnsc.png")):
...
def train_evaluate_fold_clinical(params, train_loader, val_loader, categories, num_continuous, save_dir, device='cuda'):
    ...
    def create_clinical_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "c-solo-stratification-hnsc.png")):
...
def train_evaluate_end_to_end_fusion_late(
    ...
    def create_vae_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "metalearning-stratification-hnsc.png")):
        """
        ...
        plt.title('CP-fuse-metalearning-HNSC \nProgression-Free Survival')
        plt.xlabel('Time (days)')
        plt.ylabel('Survival Probability')
        plt.grid(True)
        plt.text(0.5, 0.2, f'Log-rank\nP={p_value:.3f}', 
    ...
    for epoch in range(unified_epochs):
        ...
        with torch.no_grad():
            ...
            for batch in unified_val_loader:
                ...
                risk_val, _, _, _, _ = unified_model(x_cat, x_num, x_wsi, mask_wsi)
                risk_val = torch.nan_to_num(risk_val, nan=0.0, posinf=1e6, neginf=-1e6)
                risk_val = torch.clamp(risk_val, -1e6, 1e6) 
                risks_list.append(risk_val.cpu())
                times_list.append(t_b)
                events_list.append(e_b)
...
def run_joint_experiment_standard_cv(clinical_data_yes_id, clinical_param_grid, embedding_dir, json_path, wsi_param_grid, sr, filter_substring="DX1"):
    ...
    clinical_common.drop(columns=columns_to_drop, inplace=True, errors='ignore')
    ...
    clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_T'] = \
        clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_T'].apply(encode_t)
    clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_N'] = \
        clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_N'].apply(encode_n)
    clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_M'] = \
        clinical_common['patient.stage_event.tnm_categories.pathologic_categories.pathologic_M'].apply(encode_m)
    clinical_common['patient.stage_event.pathologic_stage'] = \
        clinical_common['patient.stage_event.pathologic_stage'].apply(encode_stage)

    clinical_common['patient.days_to_birth'].fillna(
        clinical_common['patient.days_to_birth'].mean(), inplace=True)
    clinical_common['age_of_patient'] = clinical_common['patient.days_to_birth'] / 365.25

    print(clinical_common['age_of_patient'])
    ...
    clinical_common[tobacco_alcohol_cols] = clinical_common[tobacco_alcohol_cols].fillna(-1)
    ...
    clinical_common[pathologic_cols] = clinical_common[pathologic_cols].fillna(-1)

    categorical_cols = clinical_common.select_dtypes(include=['object']).columns

    clinical_common[categorical_cols] = clinical_common[categorical_cols].fillna("missing")
 ...
 ##########new done#############################
    ...
    fold_splits = list(skf.split(clinical_common, clinical_common["PFI"]))
    ...
    #orig_embeddings_dir = os.path.expanduser('/Data/Juan/local_embeddings/TCGA-HNSC-embeddings/')
    #orig_embeddings_dir = os.path.expanduser('/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-HNSC-embeddings/')
    orig_embeddings_dir = os.path.abspath('../local_embeddings/TCGA-HNSC-embeddings/')    
...
if __name__ == "__main__":
    ...
    #embedding_dir = "/Data/Juan/local_embeddings/TCGA-HNSC-embeddings/"
    #embedding_dir = "/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-HNSC-embeddings/"
    embedding_dir = '../local_embeddings/TCGA-HNSC-embeddings/'        
    json_path = "svs_patient_map_PFI_hnsc.json"
