...
#root_folder = '/Data/Juan/local_embeddings/TCGA-BLCA/clinical'
#root_folder = '/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-BLCA/clinical'
root_folder = '../local_embeddings/TCGA-BLCA/clinical'
...
df_blca = df[df['type'].str.contains("BLCA", na=False)]
...
df_hiv = df_hiv_in[df_hiv_in['Study'].str.contains("BLCA", na=False)]
...
def train_evaluate_wsi(params, train_loader, val_loader, save_dir):
    ...
    # ... [previous code remains the same until just before the return statement] ...

    # --- Add Risk Stratification Plot Generation Here --- #
    def create_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "p-solo-stratification-blca.png")):

        
        # Stratify into high/low risk (median split)
        risk_threshold = np.median(risks)
        high_risk = risks > risk_threshold
        low_risk = ~high_risk
        ...
        # Plot low risk
        kmf.fit(times[low_risk], events[low_risk], label=f'Low-risk (n={sum(low_risk)})')
        kmf.plot_survival_function(ci_show=False, color='blue', ax=ax)
        ...
        plt.title('P-solo-BLCA \nProgression-Free Survival')
        plt.xlabel('Time (days)')
        plt.ylabel('Survival Probability')
        plt.grid(True)
        
        # Add p-value annotation
        plt.text(0.5, 0.2, f'Log-rank\nP={p_value:.3f}', 
        ...
        print(f"IMAGE GENERATED FOR STRATIFICATION OF P-SOLO: {save_path}")
        # After creating high_risk/low_risk masks but before plotting:
        n_high_events = int(events[high_risk].sum())  # Number with events in high-risk
        n_low_events = int(events[low_risk].sum())   # Number with events in low-risk

        print(f"\nHigh-risk group: {sum(high_risk)} patients, {n_high_events} with events")
        print(f"Low-risk group: {sum(low_risk)} patients, {n_low_events} with events")

    # Generate the stratification plot using validation data
    create_stratification_plot(risks_val, times_val, events_val)
    assert 1==0
...
def train_evaluate_end_to_end_fusion_late(
    ...
    # We can compute IBS similarly to your existing code
    kmf = KaplanMeierFitter()
    # Fit for the censoring distribution (events == 0 means "censored")
    kmf.fit(val_times, event_observed=(~val_events))

    time_points = np.sort(np.unique(val_times))
    n = len(val_times)
    lower_bound = 5 / (np.sqrt(n) * np.log(n))  # to avoid extreme small denominators

    # Build a dict of inverse probability of censoring weights
    ipcw_weights = {}
    for t in time_points:
        G_t = kmf.survival_function_at_times(t).values[0]
        G_t = max(G_t, lower_bound)
        ipcw_weights[t] = 1 / np.clip(G_t, 1e-8, None)

    # Compute Brier scores for each time point & average
    brier_scores = []
    for t in time_points:
        # baseline_cumulative_hazard(t) = H_0(t)
        H_t = baseline_cumulative_hazard(t)
        # As in your snippet: S_t = exp(-H_t) if your "risk" is already log-risk or something else.
        # If your model is predicting a linear predictor, you might do S_t = exp(-H_t * risk).
        # Adjust as needed based on your actual pipeline.
        S_t = np.exp(-H_t)  # per your snippet: "DO NOT multiply again by risks_val"

        # (val_times > t) is 1 if the patient survived past t, else 0
        # multiply by ipcw_weights[t] if censored
        censor_mask = (val_events == 0)
        brier_t = np.mean(
            (S_t - (val_times > t))**2 * np.where(censor_mask, ipcw_weights[t], 1)
        )
        brier_scores.append(brier_t)

    ibs = np.mean(brier_scores)

    # Final IBS with best model
    # If you do want a final C-index call (redundant since best_cindex is known):
    final_cindex = concordance_index_censored(
        event_indicator=val_events,
        event_time=val_times,
        estimate=val_risks
    )[0]
...
def train_evaluate_fold_clinical(params, train_loader, val_loader, categories, num_continuous, save_dir, device='cuda'):
    ...
    def create_clinical_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "c-solo-stratification-blca.png")):

        import matplotlib.pyplot as plt
        from lifelines import KaplanMeierFitter
        from lifelines.statistics import logrank_test
        """
        Identical to WSI version but for clinical data
        Generates KM plot with median split and log-rank test
        """
        # Stratify into high/low risk (median split)
        risk_threshold = np.median(risks)
        high_risk = risks > risk_threshold
        ...
        print(f"IMAGE GENERATED FOR CLINICAL STRATIFICATION: {save_path}")
        
        # Event counts
        n_high_events = int(events[high_risk].sum())
        n_low_events = int(events[low_risk].sum())

        print(f"\nHigh-risk group: {sum(high_risk)} patients, {n_high_events} with events")
        print(f"Low-risk group: {sum(low_risk)} patients, {n_low_events} with events")

    create_clinical_stratification_plot(risks_val, times_val, events_val)
...
def train_evaluate_end_to_end_fusion(
    ...
    # Compute IBS for best model
    kmf = KaplanMeierFitter()
    kmf.fit(final_times, event_observed=final_events == 0)
    time_points = np.sort(np.unique(final_times))
    ipcw_weights = {}
    lower_bound = 5 / (np.sqrt(len(final_times)) * np.log(len(final_times)))
    for t in time_points:
        G_t = kmf.survival_function_at_times(t).values[0]
        G_t = max(G_t, lower_bound)
        ipcw_weights[t] = 1 / np.clip(G_t, 1e-8, None)
    ...
    for t in time_points:
        H_t = baseline_cumulative_hazard(t)
        S_t = np.exp(-H_t)
        censor_mask = (final_events == 0)
        brier_t = np.mean(
            (S_t - (final_times > t)) ** 2 * np.where(censor_mask, ipcw_weights[t], 1)
        )
        ibs_values.append(brier_t)

    best_ibs = np.mean(ibs_values)
...
def train_evaluate_end_to_end_fusion_vae(
    ...
    def create_vae_stratification_plot(risks, times, events, save_path=os.path.join(save_dir, "vae-stratification-blca.png")):
        """
        Generates KM plot for VAE fusion model predictions
        Identical structure to clinical/WSI versions but with VAE-specific labeling
        """
        import matplotlib.pyplot as plt
        from lifelines import KaplanMeierFitter
        from lifelines.statistics import logrank_test
        ...
        # Event counts
        print(f"\nVAE Fusion Risk Groups:")
        print(f"High-risk: {events[high_risk].sum()}/{len(high_risk)} events")
        print(f"Low-risk: {events[low_risk].sum()}/{len(low_risk)} events")
        print(f"KM plot saved to: {save_path}")

    create_vae_stratification_plot(val_risks, val_times, val_events)
...
def run_joint_experiment_standard_cv(clinical_data_yes_id, clinical_param_grid, embedding_dir, json_path, wsi_param_grid, sr, filter_substring="DX1"):
    """
    ...
    numeric_cols = [
        ...
        'patient.eastern_cancer_oncology_group',
        'patient.karnofsky_performance_score',
        'patient.stopped_smoking_year',
        'patient.age_began_smoking_in_years',
        'patient.person_occupation_years_number',
        'patient.stage_event.gleason_grading.gleason_score',
        'patient.history_non_muscle_invasive_blca.resp_maint_from_bcg_admin_month_dur'
    ]
    ...
    categorical_cols = [
        ...
        'patient.person_concomitant_prostate_carcinoma_occurrence_indicator',
        'patient.history_non_muscle_invasive_blca.hist_of_non_mibc',
        'patient.person_occupation_description_text',
        'patient.occupation_primary_job',
        'patient.person_primary_industry_text',
        'patient.bladder_carcinoma_extracapsular_extension_status',
        'patient.metastatic_site_list.metastatic_site',
        'patient.person_concomitant_prostate_carcinoma_pathologic_t_stage',
        'patient.anatomic_neoplasm_subdivision_4',
        'patient.history_non_muscle_invasive_blca.non_mibc_tx',
        'patient.chemical_exposure_text',
        'patient.history_non_muscle_invasive_blca.mibc_90day_post_resection_bcg',
        'patient.history_non_muscle_invasive_blca.induction_course_complete',
        'patient.history_non_muscle_invasive_blca.maint_therapy_course_complete',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_2.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_2.cancer_diagnosis_cancer_type_icd9_text_name',
        'patient.history_non_muscle_invasive_blca.complete_response_observed',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_3.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_3.cancer_diagnosis_cancer_type_icd9_text_name',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_4.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_4.cancer_diagnosis_cancer_type_icd9_text_name',
        'patient.other_metastatic_site',
        'patient.anatomic_neoplasm_subdivision_5',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_5.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_5.cancer_diagnosis_cancer_type_icd9_text_name',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_6.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_6.cancer_diagnosis_cancer_type_icd9_text_name',
        'patient.anatomic_neoplasm_subdivision_6',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_2.cancer_diagnosis_cancer_type_icd9_text_name_2',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history.cancer_diagnosis_cancer_type_icd9_text_name_2',
        'patient.metastatic_site_list.metastatic_site_2',
        'patient.metastatic_site_list.metastatic_site_3',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_4.cancer_diagnosis_cancer_type_icd9_text_name_2',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history_3.cancer_diagnosis_cancer_type_icd9_text_name_2',
        'patient.init_pathology_dx_method_other',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history.family_member_relationship_type',
        'patient.blood_relative_cancer_history_list.blood_relative_cancer_history.cancer_diagnosis_cancer_type_icd9_text_name'
    ]
    ...
    #orig_embeddings_dir = os.path.expanduser('/Data/Juan/local_embeddings/TCGA-BLCA-embeddings/')
    #orig_embeddings_dir = os.path.expanduser('/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-BLCA-embeddings/')
    orig_embeddings_dir = os.path.abspath('../local_embeddings/TCGA-BLCA-embeddings/')    
...
if __name__ == "__main__":
    ...
    #embedding_dir = "/Data/Juan/local_embeddings/TCGA-BLCA-embeddings/"
    #embedding_dir = "/home/sorkwos/projects/rrg-senger-ab/multimodality/contrastive_learning/TCGA-HNSC-data/local_embeddings/TCGA-BLCA-embeddings/"
    embedding_dir = '../local_embeddings/TCGA-BLCA-embeddings/'    
    json_path = "svs_patient_map_PFI_blca.json"
